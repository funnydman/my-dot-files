...
| Idle | hypridle | `hypr/hypridle.conf` (dim 600s, lock 700s, suspend 3600s) |
...
| `lock.sh` | Mod+Escape | Crash-recovery wrapper for hyprlock (restarts on KVM hotplug crash) |
...
| `volume.sh` | XF86Audio* | Volume control (legacy, now uses wpctl directly in hyprland.conf) |
...
## Legacy Configs (i3 era, archived but tracked)

- `i3/config`, `polybar/config.ini`, `picom/picom.conf`, `dunst/dunstrc`
- `i3/scripts/`, `scripts/switcher.py`, `scripts/next_available.py`
- `zathura/zathurarc` (replaced by Sioyek)
...
- **hyprlock + Russian layout**: Lock screen inherits active layout. Fixed: `switchxkblayout all 0` runs before hyprlock in both hyprland.conf and hypridle.conf
- **hyprlock + KVM hotplug**: KVM switch disconnects/reconnects HDMI, crashing hyprlock (SIGABRT from invalid EGL surface). Fixed: `scripts/lock.sh` wrapper auto-restarts hyprlock + `allow_session_lock_restore = true`. Upstream fix in PR #845 (merged, not in stable 0.9.2)
- **KVM suspend on switch**: KVM switch-away disconnects HDMI, logind sees lid closed + AC power + no display → suspends immediately. Fixed: `HandleLidSwitchExternalPower=ignore` in `/etc/systemd/logind.conf` (hypridle 3600s timeout still handles real-idle suspend)
...
- `migration2hypr.md` - Original i3 to Hyprland migration plan
